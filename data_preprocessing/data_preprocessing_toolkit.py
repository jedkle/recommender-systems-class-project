...
class DataPreprocessingToolkit(object):
    ...
    def add_length_of_stay(self, df):
        ...
        Day_Stay = (df["date_to"] - df["date_from"]).dt.days
        df.loc[:,"length_of_stay"] = Day_Stay
        return df
    ...
    def add_night_price(self, df):
        # Write your code here
        Night_Cost = df["accomodation_price"]/df["n_rooms"]
        Night_Cost = Night_Cost / df["length_of_stay"]
        df.loc[:,"night_price"] = round(Night_Cost, 2)
        return df
    ...
    def map_night_price_to_room_segment_buckets(self, df):
        ...
        night_prices.columns = ['room_group_id', 'one_room_night_price']
        df = pd.merge(df, night_prices, on=['room_group_id'], how='left')
        df.loc[:, 'room_segment'] = df['one_room_night_price'].apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
        df = df.drop(columns=['one_room_night_price'])
        return df
